...
function parseWeightsParam(input: unknown): Record<string, number> {
  const defaults: Record<string, number> = { Collection: 1, Style: 0.8, Color: 0.3, UpsellRank: 1, PriceDelta: 0.7 };
...
function parseNumeric(value: string): number {
  // Extract first number in string, e.g. "rank_3" -> 3
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    const mode = String(req.query.mode || 'similar').toLowerCase();
    const take = Number(req.query.take || req.query.limit || 8);
    const sort = String(req.query.sort || 'none').toLowerCase(); // 'price' | 'none' | 'score'
    ...
    const minPriceFactor = Number(req.query.minPriceFactor || (mode === 'upsell' ? 1.1 : 0));

    // Defaults per mode
    const defaultFacetCodes = mode === 'accessory' ? ['Collection'] : ['Collection', 'Style'];
    ...
    const excludeProductType = excludeProductTypeParam || (mode === 'accessory' ? currentProductType : '');
    ...
    const vars: Record<string, any> = {
      ...
      take: take + 12, // fetch more then trim after scoring
    };
    ...
    // If only simple sort requested and no scoring/exclusion needed
    const needsScoring = Boolean(excludeProductType) || sort === 'score' || mode === 'upsell';

    if (!needsScoring && sort === 'price') {
      items.sort((a, b) => Number(a.priceWithTax ?? 0) - Number(b.priceWithTax ?? 0));
    }

    let limitedItems = items;

    if (needsScoring) {
      // Fetch candidate facetValues to compute score and optionally exclude ProductType
      const candidateQ = `
        query C($id: ID!) {
          product(id: $id) {
            id
        ...
        .map(({ it, cand }) => {
          ...
          for (const code of Object.keys(weights)) {
            ...
            if (currentIds && candIds) {
              // any overlap yields weight points
              const overlap = [...currentIds].some((id) => candIds.has(id));
              if (overlap) score += weights[code] || 0;
            }
          }

          // UpsellRank boost (from candidate facet value name numeric)
          ...
          // PriceDelta boost for upsell mode
          if (mode === 'upsell' && baseMinPrice > 0 && weights['PriceDelta']) {
            const price = Number(it.priceWithTax ?? 0);
            const factor = price / baseMinPrice;
            if (minPriceFactor > 0 && factor < minPriceFactor) {
              return null; // exclude too-cheap candidates for upsell
            }
            if (factor > 1) {
      ...
      limitedItems = scored.slice(0, take);
    } else {
      limitedItems = items.slice(0, take);
    }

    const crossSells = limitedItems.map((it: any) => ({
      productId: it.productId,
      slug: it.slug,
    ...
    res.status(200).json({
      ...
      crossSells,
      params: { mode, take, sort: needsScoring ? 'score' : sort, facetCodes, excludeProductType: excludeProductType || undefined, weights, minPriceFactor: minPriceFactor || undefined },
    });
  } catch (e: any) {
